...
int _sock::read_packet(int socketId, ERegisterPacket* outPacket) 
{
    ...
    n = Socket::Read(socketId, (u8*) &type, sizeof(EPacketType));
    if (n <= 0) { return n; }
    Socket::Read(socketId, (u8*) &ID, sizeof(ERegisterPacket::PackId));
    if (n <= 0) { return n; }

    size_t dataLen = 0;
    Socket::Read(socketId, (u8*)&dataLen, sizeof(size_t));
    if (n <= 0) { return n; }
    u8* bodyData = new u8[dataLen];
    memset(bodyData, 0, dataLen);
    Socket::Read(socketId, bodyData, dataLen);
