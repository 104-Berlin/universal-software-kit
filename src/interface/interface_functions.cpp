...
shared::ESharedError shared::CreateResource(EResourceBase* data) 
{
...
ERef<EResourceBase> shared::GetResource(EResourceBase::t_ID id) 
{
    return StaticSharedContext::instance().GetRegisterConnection().Send_GetResourceData(id);
}

EVector<ERef<EResourceBase>> shared::GetLoadedResource(const EString& resourceType) 
{
